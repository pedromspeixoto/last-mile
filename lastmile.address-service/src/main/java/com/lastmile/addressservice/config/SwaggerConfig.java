package com.lastmile.addressservice.config;
...
public class SwaggerConfig {
    ...
    public Docket api() {
        return new Docket(DocumentationType.SWAGGER_2).apiInfo(apiInfo()).enable(swaggerProperties.isEnabled()).select()
                .apis(RequestHandlerSelectors.basePackage("com.lastmile.addressservice.controller"))
    ...
    private ApiInfo apiInfo() {
        return new ApiInfoBuilder().title("Address Service - Last Mile LS").description("LastMile LS")
                // Author information
