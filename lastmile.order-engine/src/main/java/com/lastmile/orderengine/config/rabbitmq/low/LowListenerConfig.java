package com.lastmile.orderengine.config.rabbitmq.low;
...
@Configuration
public class LowListenerConfig {
	...
	@Value("${rabbitmq.low.queue}")
	private String queue;

	@Value("${rabbitmq.low.durable}")
	private Boolean durable;

	@Value("${rabbitmq.low.routing-keys}")
    ...
    @Bean
    public List<Binding> lowQueueBindings() {

		List<Binding> bindings = new ArrayList<Binding>();

		for (String routingKey : routingKeys) {
			bindings.add(BindingBuilder.bind(this.lowQueue()).to(this.exchange).with(routingKey));
		}
    ...
    @Bean
    public SimpleRabbitListenerContainerFactory lowListenerFactory() {
    ...
    public void amqpDeclarations() {
		...
		this.amqpAdmin.declareQueue(this.lowQueue());
		this.amqpAdmin.declareExchange(this.exchange);

		for (Binding binding : this.lowQueueBindings()) {
