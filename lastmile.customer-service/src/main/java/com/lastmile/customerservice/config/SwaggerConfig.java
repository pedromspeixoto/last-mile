package com.lastmile.customerservice.config;
...
public class SwaggerConfig {
    ...
    public Docket api() {
        return new Docket(DocumentationType.SWAGGER_2).apiInfo(apiInfo()).enable(swaggerProperties.isEnabled()).select()
                .apis(RequestHandlerSelectors.basePackage("com.lastmile.customerservice.controller"))
    ...
    private ApiInfo apiInfo() {
        return new ApiInfoBuilder().title("Customer Service - Last Mile LS").description("LastMile LS")
                // Author information
